...
export function AddChapterModal({ isOpen, onClose, onSave, courseId, editingChapter }: AddChapterModalProps) {
  ...
  useEffect(() => {
    if (editingChapter) {
      ...
      setDuration(editingChapter.duration);
    } else {
      setName('');
      setDescription('');
      setDuration('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const chapter: Chapter = {
      _id: editingChapter?._id || `chapter-${Date.now()}`,
      name,
      description,
      duration: duration || '00:00',
      courseId,
      videos: editingChapter?.videos || []
    };
          ...
          >
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-xl">
              ...
              <div className="border-b border-neutral-200 px-6 py-4 flex items-center justify-between">
                <h2 className="text-neutral-900">{editingChapter ? 'Edit Chapter' : 'Add New Chapter'}</h2>
                <button
                  onClick={onClose}
